...
import { PlusCircle, MessageSquare, ChevronLeft, ChevronRight, Trash2, Edit } from 'lucide-react';
...
const ChatSidebar = ({ activeChat, onChatSelect, onNewChat }: ChatSidebarProps) => {
  ...
  const handleUpdateTitle = async (chatId: string) => {
    ...
    try {
      ...
      setChats((prev) => prev.map(chat => chat._id === chatId ? { ...chat, title: newTitle } : chat));
      setEditingChat(null);
    } catch (error) {
              ...
              <div key={chat._id} className={`flex items-center rounded-md px-3 py-2 text-sm transition-colors duration-200 ${activeChat === chat._id ? "bg-gray-700 text-white" : "text-gray-300 hover:bg-gray-700 hover:text-white"}`}>
                ...
                {!collapsed && (
                  <div className="flex items-center space-x-2">
                    ...
                    <button onClick={() => handleDeleteChat(chat._id)} className="text-red-400 hover:text-red-600">
                      <Trash2 size={16} />
                    </button>
                  </div>
                )}
