...
import { PlusCircle, MessageSquare, ChevronLeft, ChevronRight, Trash2, Edit, Loader2 } from 'lucide-react';
...
const ChatSidebar = ({ activeChat, onChatSelect, onNewChat }: ChatSidebarProps) => {
  ...
  const handleNewChat = async () => {
    ...
    try {
      ...
      const data = await response.json();
      onNewChat();
      setChats((prev) => [{ _id: data.chatId, title: 'New Chat', timestamp: new Date() }, ...prev]);
    } catch (error) {
  ...
  const handleDeleteChat = async (chatId: string) => {
    ...
    try {
      await fetch(`/api/chats?userId=${userId}&chatId=${chatId}`, { method: 'DELETE' });
      setChats((prev) => {
    ...
    <div className={`h-screen flex flex-col bg-[#1E1E1E] border-r border-gray-800 transition-all duration-300 ease-in-out ${collapsed ? "w-16" : "w-72"}`}>
      ...
      <div className="px-3 mb-4">
        <button 
          ...
          className={`w-full flex items-center rounded-md py-2 bg-purple-600 hover:bg-purple-700 text-white transition-colors duration-200 ${collapsed ? "justify-center px-0" : "justify-start pl-3 pr-4"}`}
        >
          {isCreatingChat ? (
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-2 space-y-1">
          {loading ? (
            ...
            chats.map((chat) => (
              <div key={chat._id} className={`flex items-center rounded-md px-3 py-2 text-sm transition-colors duration-200 ${activeChat === chat._id ? "bg-gray-700 text-white" : "text-gray-300 hover:bg-gray-700 hover:text-white"}`}>
                {editingChat === chat._id ? (
                  ...
                  <button onClick={() => onChatSelect(chat._id)} className="flex-1 text-left truncate">
                    <MessageSquare size={16} className="flex-shrink-0 inline-block mr-2" />
                    {!collapsed && chat.title}
                  </button>
                )}
                {!collapsed && (
                  <div className="flex items-center space-x-2">
                    <button onClick={() => { setEditingChat(chat._id); setNewTitle(chat.title); }} className="text-blue-400 hover:text-blue-600">
                      <Edit size={16} />
                    </button>
                    <button 
